...
def merge_video_audio(video_path, audio_path, output_path):
    """
    Precisely merges a video clip with an audio file.
    ...
    try:
        ...
        # Calculate the common duration to avoid exceeding either clip's length
...
if __name__ == "__main__":
    # Define paths for our verified typewriter scene
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    
    video_in = os.path.join(BASE_DIR, "output", "clips", "verify_scene001.mp4")
    audio_in = os.path.join(BASE_DIR, "output", "clips", "scene_1_foley.wav")
    video_out = os.path.join(BASE_DIR, "output", "clips", "verify_scene001_with_foley.mp4")
